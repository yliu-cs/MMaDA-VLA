...
@dataclass
class MMaDAVLADiffusionInferenceOutput:
    cur_vision_features: torch.Tensor
    vision_sequences: torch.Tensor
    vision_history: List[torch.Tensor]
...
def rl_generate(
    ...
    mask_token_id: int = 126336,
    prompt: Prompting = None,
) -> MMaDAVLADiffusionInferenceOutput:
    num_new_special_tokens = 0
    action_input_ids = input_ids[:, -(mmadavla.config.action_num_vq_tokens + 1):-1].clone()
    action_input_ids = torch.where(action_input_ids == mask_token_id, mask_token_id, action_input_ids - len(prompt.tokenizer) - prompt.vision_codebook_size - num_new_special_tokens)
    vision_input_ids = input_ids[:, -(mmadavla.config.vision_num_vq_tokens + mmadavla.config.action_num_vq_tokens + 3):-(mmadavla.config.action_num_vq_tokens + 3)].clone()
    vision_input_ids = torch.where(vision_input_ids == mask_token_id, mask_token_id, vision_input_ids - len(prompt.tokenizer) - num_new_special_tokens)

    cur_vision_hidden_states, vision_history, action_history = [], [], []
    for step in range(timesteps):
        outputs = mmadavla(input_ids, attention_bias=attention_bias, output_hidden_states=True)
        hidden_states, logits = outputs.hidden_states, outputs.logits
        # cur_vision_hidden_states = torch.stack([hidden_states[-1][:, 1, :], hidden_states[-1][:, 2 + mmadavla.config.vision_num_vq_tokens, :]], dim=1)
        # print("-" * 100)
        cur_vision_hidden_states.append(hidden_states[-1][:, 1:3 + mmadavla.config.vision_num_vq_tokens, :])
        # print("=" * 100)
        action_logits = logits[:, -(mmadavla.config.action_num_vq_tokens + 1):-1, len(prompt.tokenizer) + num_new_special_tokens + prompt.vision_codebook_size:len(prompt.tokenizer) + num_new_special_tokens + prompt.vision_codebook_size + prompt.action_codebook_size]
        vision_logits = logits[:, -(mmadavla.config.vision_num_vq_tokens + mmadavla.config.action_num_vq_tokens + 3):-(mmadavla.config.action_num_vq_tokens + 3), len(prompt.tokenizer) + num_new_special_tokens:len(prompt.tokenizer) + num_new_special_tokens + prompt.vision_codebook_size]
        ...
        action_mask_len = (mmadavla.config.action_num_vq_tokens * mask_ratio).floor().unsqueeze(0).to(action_logits.device)
        action_mask_len = torch.max(
            torch.tensor([1], device=action_logits.device),
            torch.min(action_unknown_map.sum(dim=-1, keepdim=True) - 1, action_mask_len)
        )
        vision_mask_len = (mmadavla.config.vision_num_vq_tokens * mask_ratio).floor().unsqueeze(0).to(vision_logits.device)
        vision_mask_len = torch.max(
            torch.tensor([1], device=vision_logits.device),
            torch.min(vision_unknown_map.sum(dim=-1, keepdim=True) - 1, vision_mask_len)
        )
        ...
        input_ids[:, -(mmadavla.config.action_num_vq_tokens + 1):-1] = torch.where(
            action_masking,
            mask_token_id,
            action_sampled_ids + len(prompt.tokenizer) + num_new_special_tokens + prompt.vision_codebook_size
        )
        action_input_ids = torch.where(action_masking, mask_token_id, action_sampled_ids)
        input_ids[:, -(mmadavla.config.vision_num_vq_tokens + mmadavla.config.action_num_vq_tokens + 3):-(mmadavla.config.action_num_vq_tokens + 3)] = torch.where(
            vision_masking,
            mask_token_id,
            vision_sampled_ids + len(prompt.tokenizer) + num_new_special_tokens
        )
        ...
        vision_history.append(vision_masking.clone().cpu())
        action_history.append(action_masking.clone().cpu())
    stacked_hidden = torch.stack(cur_vision_hidden_states, dim=0)
    cur_vision_features = stacked_hidden.mean(dim=0).mean(dim=1).cpu()
    return MMaDAVLADiffusionInferenceOutput(
        cur_vision_features=cur_vision_features,
        vision_sequences=vision_sampled_ids,
        vision_history=vision_history,
        action_sequences=action_sampled_ids,
