...
def main(args: Namespace) -> None:
    ...
    with torch.no_grad():
        gen_action_ids_list, gen_vision_ids_list= rold.generate(
            input_ids=input_ids,
    ...
    print(f"{gt_action_ids.shape=} {gt_action_ids.flatten().min().item()=} {gt_action_ids.flatten().max().item()=}")
    gt_action_ids_list = gt_action_ids.flatten().detach().cpu().numpy().tolist()
    print(" ".join(list(map(str, gt_action_ids_list))))

    for step, (step_action_ids, step_vision_ids) in enumerate(zip(gen_action_ids_list, gen_vision_ids_list)):
        ...
        for gt_action_id, step_action_id in zip(gt_action_ids_list, step_action_ids_list):
            print_id_list.append(f"[green]{step_action_id}[/green]" if gt_action_id == step_action_id else f"[red]{step_action_id}[/red]")
        ...
        print(f"{' '.join(print_id_list)} [orange]{acc=:.2f}[/orange]")
        ...
        print(f"[{step + 1}/{args.timesteps}] {mse=}")
